...
class Main extends Sprite
{
	...
	private function setupGame():Void
	{
		...
		if(fpsVar != null) {
			//fpsVar.scaleX = fpsVar.scaleY = ClientPrefs.data.fpsCounterSize;		  
		...
		FlxG.signals.gameResized.add(function (w, h) {
			if(fpsVar != null)
				fpsVar.positionFPS(10, 5, Math.min(Lib.current.stage.stageWidth / FlxG.width, Lib.current.stage.stageHeight / FlxG.height));
